...
import * as __nodeAsyncHooksExternal from "node:async_hooks";
...
const __requestContextModule = (() => {
  const { AsyncLocalStorage } = __nodeAsyncHooksExternal;
  ...
  function readRequestContext(store) {
    if (!store) {
      return {};
    }
    if (typeof store.get === "function") {
  ...
  return { createRequestContextStore, readRequestContext };
})();
...
const __serverEntryModule = (() => {
  const { createApp: createBaseApp, defineApp: defineBaseApp, readSnapshot } = __appModule;
  const { createServerRegistry } = __serverRegistryModule;
  function createApp(appOrDefinition = Async, options = {}) {
  ...
  const Async = defineApp();
  return { createApp, defineApp, Async, readSnapshot };
})();
...
const { Async: Async } = __serverEntryModule;
const { createApp: createApp } = __serverEntryModule;
const { defineApp: defineApp } = __serverEntryModule;
...
const { createRequestContextStore: createRequestContextStore } = __requestContextModule;
...
const { createServerRegistry: createServerRegistry } = __serverRegistryModule;
