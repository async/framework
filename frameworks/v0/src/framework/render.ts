...
export function render(
  element?: HTMLElement | RenderConfig | ((config: RenderConfig) => HTMLElement),
  config?: RenderConfig,
) {
  if (element instanceof HTMLElement && element.hasAttribute("data-container")) {
  ...
  const renderConfig: RenderConfig = config ?? {};
  let domRoot = renderConfig.root;
  if (!domRoot) {
