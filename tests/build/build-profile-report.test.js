...
test("Vite plugin spike exposes virtual plan and bootstrap transform", () => {
  ...
  assert.deepEqual(plugin.config({}, { mode: "development" }), { optimizeDeps: { exclude: [...frameworkOptimizeDepsExclude] } });
...
test("Vite plugin configures client asset builds only in client mode", () => {
  ...
  assert.deepEqual(plugin.config({}, { mode: "client" }), {
    optimizeDeps: { exclude: [...frameworkOptimizeDepsExclude] },
    build: {
      outDir: "public/static",
      copyPublicDir: false,
      rollupOptions: {
...
test("Vite plugin composes Hono dev server when server options are enabled", async () => {
  ...
  assert.deepEqual(plugin.config({ appType: "spa" }, { mode: "development" }), { optimizeDeps: { exclude: [...frameworkOptimizeDepsExclude] } });
