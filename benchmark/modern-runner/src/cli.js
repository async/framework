...
function printHelp() {
  console.log(`Usage: node src/cli.js [--mode smoke|trace] [--framework keyed/react ...] [--benchmark 01_ ...] [--iterations 3]

Defaults:
  mode       trace
  framework  keyed/js-only keyed/react keyed/qwik-v1 keyed/qwik-v2 keyed/solid-v1 keyed/solid-v2
...
function frameworkMatches(framework, selected) {
  if (selected.length === 0) return DEFAULT_FRAMEWORKS.includes(`${framework.type}/${framework.directory}`);
  return selected.includes(`${framework.type}/${framework.directory}`) || selected.includes(framework.directory);
}

async function loadFrameworks(host, port, selectedFrameworks) {
  const rows = await fetchJson(`http://${host}:${port}/ls`);
  return rows
    .filter((row) => row.type === "keyed" && frameworkMatches(row, selectedFrameworks))
    .map((row) => ({
      name: row.directory,
      type: row.type,
      keyed: row.type === "keyed",
      fullName: row.frameworkVersionString,
      uri: `frameworks/${row.type}/${row.directory}${row.customURL ?? ""}`,
